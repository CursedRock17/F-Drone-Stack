                       ...
                       float lin_velocity[3], float ang_velocity[3])
{
  ...
  micro_ros_string_utilities_set(odom_msg->child_frame_id, "/odom");
  ...
  RCCHECK(rcl_publish(&odom_pub, odom_msg, NULL));
}
...
void odom_setup()
{
  ...
  RCCHECK(rcl_init_options_init(&init_options, odom_alloc));
  ...
  RCCHECK(rclc_publisher_init(&odom_pub, 
                              &odom_node,
  ...
  if (!micro_ros_utilities_create_message_memory(
      ROSIDL_GET_MSG_TYPE_SUPPORT(nav_msgs, msg, Odometry), 
      &odom_msg, (micro_ros_utilities_memory_conf_t) {}))
      {
        printf("Message Memory Error");
      }
