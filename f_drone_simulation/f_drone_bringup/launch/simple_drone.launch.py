...
# Drone teleop with live URDF and TF visualization through Foxglove.
def generate_launch_description():
    ...
    with open(os.path.join(description_dir, "models", "drone", "urdf", "drone.urdf")) as source:
    ...
    world = os.path.join(f_drone_gazebo, "worlds", "simple_drone.sdf")
    ...
    gui_config = os.path.join(pkg_share_dir, "config", "simple_drone_gui.config")
    ...
    # drone, regardless of the entity name given in the world file.
    ros_gz_bridge = Node(
        ...
        parameters=[{
            'config_file': os.path.join(
                pkg_share_dir, 'config', 'simple_drone_bridge.yaml'),
        }],
        output='screen'
    )
    ...
    teleop_lateral = Node(
        ...
        name='teleop_lateral',
        parameters=[{
            'input_topic': '/drone/teleop/twist',
            'output_topic': '/drone/gazebo/command/twist',
